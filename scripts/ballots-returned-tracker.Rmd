...
today <- Sys.Date() #get todays date
...
row_with_names <- as.numeric(which(grepl("County Name", ballots$...1))) #which row has the actual column names

names(ballots) <- ballots[3,] #fill in the column names with the actual columnn names from that row
...
ballots_clean <- ballots %>% 
  filter(`County Name` %in% counties) %>%  #filter out only the rows that have county names
  mutate( across(
        .cols = everything(),
        ~str_replace( ., "NULL", "0" ) #replace everything that's "NULL" with a "0"
    ) ) %>% 
  select(`County Name`, `Total VBM Ballots Issued (Distinct Voters)`,`Total VBM Ballots Accepted`, `Voter Returned - Drop Box`, `Voter Returned - Drop Off Location`, `Voter Returned - Vote Center Drop Off`, `Voter Returned - Mail`, `Voter Returned - FAX`, `Voter Returned - Other`) %>%  #select only the columns we need
  mutate(`Total VBM Ballots Issued (Distinct Voters)`= as.numeric(`Total VBM Ballots Issued (Distinct Voters)`),
         `Total VBM Ballots Accepted` = as.numeric(`Total VBM Ballots Accepted`), 
         `Voter Returned - Drop Box` = as.numeric(`Voter Returned - Drop Box`), 
         `Voter Returned - Drop Off Location` = as.numeric(`Voter Returned - Drop Off Location`), 
         `Voter Returned - Vote Center Drop Off` = as.numeric(`Voter Returned - Vote Center Drop Off`), 
         `Voter Returned - Mail`= as.numeric(`Voter Returned - Mail`), 
         `Voter Returned - FAX` = as.numeric(`Voter Returned - FAX`), 
         `Voter Returned - Other` = as.numeric(`Voter Returned - Other`)) %>% #convert them all to numbers
  rename(`Ballots issued`=`Total VBM Ballots Issued (Distinct Voters)`,
         `Ballots accepted`=`Total VBM Ballots Accepted`,
         County = `County Name`) %>% 
  mutate(`Ballots returned` = `Voter Returned - Drop Box` + `Voter Returned - Drop Off Location` + `Voter Returned - Vote Center Drop Off` + `Voter Returned - Mail` + `Voter Returned - FAX` + `Voter Returned - Other`) %>% #get a number for the total ballots returned
